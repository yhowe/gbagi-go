...
const KEYMAP buttonKeymap[] = {
	{KEY_RESET,		B_VOLUME|B_MENU|B_A|B_B},
	...
	{KEY_TABFWD,	B_SELECT | B_RIGHT},
	{KEY_TABREV,	B_SELECT | B_LEFT},
	{KEY_ENTER,		B_A},
	{KEY_ESC,		B_B},
	{KEY_RIGHT,		B_RIGHT},
	{KEY_LEFT,		B_LEFT},
...
BOOL SystemInit()
{
	...
	vidpage1 = (U8 *)heap_caps_malloc(320 * 240 * sizeof(U16), MALLOC_CAP_SPIRAM);
	vidpage2 = (U8 *)heap_caps_malloc(320 * 240 * sizeof(U16), MALLOC_CAP_SPIRAM);
    BACKBUFFER = 0;
    vidPtr = (U16 *)vidpage1; //now we point our drawing buffer to the back buffer
...
void SystemUpdate()
{
	...
	#define TICKME 1000000
	#define VOICE 0
	long int lasttime = now.tv_sec * 1000000 + now.tv_usec;
	static long int next_update = 0;
	static long int next_update_snd2 = 0;
	static long int next_update_snd3 = 0;
	static long int next_second_update = 0;
	if (!next_update)
		next_update = lasttime + TICKME;
	if (!next_update_snd2)
		next_update_snd2 = lasttime + TICKME;
	if (!next_update_snd3)
		next_update_snd3= lasttime + TICKME;
	if (!next_second_update)
		next_second_update = lasttime + 1000000;

 if (lasttime > next_second_update) {
    if(++vars[vSECONDS]>=60) {
    	vars[vSECONDS]=0;
        if(++vars[vMINUTES]>=60) {
    		vars[vMINUTES]=0;
	...
	U32 len;
	S16 nfreq, ofreq;
	    if (lasttime > next_update_snd3) {
		if((len = pSnds[2][0]+(pSnds[2][1]<<8))!=0xFFFF) {
		sndWaits[2]=len;
		nfreq = (((U16)pSnds[2][2] & 0x3f) << 4) | (U16)(pSnds[2][3] & 0xf);
		if ((pSnds[2][3] & 0x80) == 0x80 && nfreq != 0)
			ofreq = 2047 * 49 / nfreq;
		else
			ofreq = 0;
//		GO.Speaker.setVolume(volume);
		GO.Speaker.tone3(ofreq, len * 10000);
		next_update_snd3 = lasttime + len * 10000;
		pSnds[2]+=5;
		}
	    }
	    if (lasttime > next_update_snd2) {
		if((len = pSnds[1][0]+(pSnds[1][1]<<8))!=0xFFFF) {
		sndWaits[1]=len;
		nfreq = (((U16)pSnds[1][2] & 0x3f) << 4) | (U16)(pSnds[1][3] & 0xf);
		if ((pSnds[1][3] & 0x80) == 0x80 && nfreq != 0)
			ofreq = 2047 * 49 / nfreq;
		else
			ofreq = 0;
		GO.Speaker.setVolume(volume);
		GO.Speaker.tone2(ofreq, len * 10000);
		next_update_snd2 = lasttime + len * 10000;
		pSnds[1]+=5;
		}
	    }
	    if (lasttime > next_update) {
	if((len = pSnds[VOICE][0]+(pSnds[VOICE][1]<<8))==0xFFFF) {
		StopSound();
	} else {
		sndWaits[VOICE]=len;
//		fprintf(stderr,"PSND1 %x %x %x %x %x\n",pSnds[0][0], pSnds[0][1], pSnds[0][2], pSnds[0][3],pSnds[0][4]);
//		fprintf(stderr,"PSND2 %x %x %x %x %x\n",pSnds[1][0], pSnds[1][1], pSnds[1][2], pSnds[1][3],pSnds[0][4]);
//		fprintf(stderr,"PSND3 %x %x %x %x %x\n",pSnds[2][0], pSnds[2][1], pSnds[2][2], pSnds[2][3],pSnds[0][4]);
//		fprintf(stderr,"PSND4 %x %x %x %x %x\n",pSnds[3][0], pSnds[3][1], pSnds[3][2], pSnds[3][3],pSnds[3][4]);
		nfreq = (((U16)pSnds[VOICE][2] & 0x3f) << 4) | (U16)(pSnds[VOICE][3] & 0xf);
		if ((pSnds[VOICE][3] & 0x80) == 0x80 && nfreq != 0)
			ofreq = 2047 * 49 / nfreq;
		else
			ofreq = 0;
		GO.Speaker.setVolume(volume);
		GO.Speaker.tone(ofreq, len * 10000);
		next_update = lasttime + len * 10000;
//		fprintf(stderr, "SPKR: %x %x %x %d %d %d\n",pSnds[VOICE][2], pSnds[VOICE][3], pSnds[VOICE][4], nfreq, ofreq,len);
		pSnds[0]+=5;
		pSnds[1]+=5;
	    }
    	}
