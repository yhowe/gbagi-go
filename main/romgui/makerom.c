...
BOOL LoadDir(BOOL SINGLE, int num)
{
	...
	if(SINGLE) {
		...
		sprintf(fname,"%s%s","./",dirNames[num]);
		if((f=fopen(fname,"rb"))==NULL) {
			ErrorMessage("Unable to open file: %s, for reading!",fname);
			return FALSE;
		}
	...
	for(i=0;i<totalEnts;i++) {
		offs = (fgetc(f)<<16)|(fgetc(f)<<8)|fgetc(f);
		fprintf(stderr, "VOL %d OFFS %x\n",i, offs);
...
BOOL PrepDirs()
{
	...
	for(vn=0;vn<16;vn++) {
		...
		sprintf(fname,"%s%svol.%d","./", namevol, vn);
		if((f=fopen(fname,"rb"))==NULL) {
        	VUSED[vn] = FALSE;
        	continue;
			//ErrorMessage("Unable to open file: \"%s\", for reading!",fname);
		...
		for(t=0;t<4;t++) {
			for(i=0;i<256;i++) {
				if(dirs[t][i].vol == vn) {
					sprintf(fname,"%s%s.%03d","./",resnames[t],i);
                	if((fr=fopen(fname,"rb"))!=NULL) {
                    	...
                    	if((fgetw(f)==0x3412)&&((fgetc(f)&0x7F)==vn)) {
                    		...
                    		memset(&dirs[t][i],-1,sizeof(dirs[t][i]));
                    	}
                    }
				}
			}
...
BOOL ProcessObject()
{
	...
	q=bGetW(p);
	if((q>l || q<3 ) || version->ver.flags & ENCRYPT_OBJ)
...
*/
    nameStart 	= bGetW(p);
    entSize 	= (version->ver.flags&AMIGA)?4:3;
...
fout = fopen ("./odroid.dat", "wb");
    ...
    *wPtr++ = 0;
    ...
    giPos += 80;
  	...
  	fwrite("AGi",3,1,fout);
    fputc(version->ver.major,fout);
    fputw(version->ver.minor,fout);
	char title[] = "LARRY1";
  	fwrite(title,strlen(title)+1,1,fout);
    for(i=strlen(title)+1;i<26;i++)
    	fputc(0,fout);
...
int main(int argc, char**argv)
{
	...
	if (argc > 1)
		snprintf(namevol, sizeof(namevol), "%s", argv[1]);
	else
		namevol[0] = 0;
	fprintf(stderr, "%s\n", namevol);

		version = FindAGIVersion("./agidata.ovl");
	...
	for(i=0;i<4;i++) {
		if(!LoadDir(version->ver.flags&SINGLE_DIR, i))
			return FALSE;
	}
	if(!PrepDirs())
		return -4;
